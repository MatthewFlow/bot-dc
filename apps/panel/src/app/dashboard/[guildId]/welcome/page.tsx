        ...
        <div className="flex w-full max-w-lg flex-col gap-4">
          ...
          <div className="rounded-xl bg-[#1a1f2e] p-5 space-y-3">
            ...
            <div className="flex gap-2">
              {[1,2,3,4].map((i) => <Skeleton key={i} className="h-7 w-20 rounded" />)}
            </div>
          </div>
...
export default function WelcomePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getGuildConfig(token, guildId), getChannels(token, guildId)])
      .then(([cfg, ch]) => { setConfig(cfg); setChannels(ch); })
  ...
  const message = tab === "welcome" ? (config.welcomeMessage ?? DEFAULT_WELCOME) : (config.goodbyeMessage ?? DEFAULT_GOODBYE);
  const field = tab === "welcome" ? "welcomeMessage" : "goodbyeMessage";
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-white/5 px-8 py-6">
        <p className="text-xs font-semibold uppercase tracking-wider text-gray-600">First Contact</p>
        <h1 className="mt-1 text-2xl font-bold text-white">
          Welcome <span className="text-[#d4a843]">&</span> Goodbye
        </h1>
        <p className="mt-1 text-sm text-gray-400">Wiadomości powitalne i pożegnalne na Twoim serwerze.</p>
      </div>
      ...
      <div className="flex flex-1 gap-6 overflow-auto p-8">
        <div className="flex w-full max-w-lg flex-col gap-4">
          ...
          <div className="rounded-xl bg-[#1a1f2e] p-5">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500">Kanał</p>
            <select value={channelId ?? ""}
              onChange={(e) => { const val = e.target.value || undefined; setConfig((c) => tab === "welcome" ? { ...c, welcomeChannelId: val } : { ...c, goodbyeChannelId: val }); }}
          ...
          <div className="rounded-xl bg-[#1a1f2e] p-5">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500">Treść (Markdown + zmienne)</p>
            <textarea ref={textareaRef} value={message}
              onChange={(e) => setConfig((c) => ({ ...c, [field]: e.target.value }))}
              rows={4} className="w-full resize-none rounded-lg bg-[#0f1117] px-4 py-3 text-sm text-white outline-none focus:ring-2 focus:ring-[#d4a843]" />
            <div className="mt-3">
        ...
        <div className="flex-1 rounded-xl bg-[#1a1f2e] p-6">
          <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-500">Tak będzie to wyglądać w Discord</p>
          <div className="rounded-lg bg-[#0d1117] p-4">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#d4a843] text-sm font-bold text-black">JH</div>
              <div className="min-w-0">
                ...
                <div className="mt-2 rounded-lg border-l-4 border-[#d4a843] bg-[#1a1f2e] p-3">
                  <p className="text-sm font-semibold text-white">{tab === "welcome" ? "🎉 Witamy na serwerze!" : "👋 Do zobaczenia!"}</p>
                  <p className="mt-1 whitespace-pre-wrap break-words text-sm text-gray-300">{resolvePreview(message)}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="mt-6">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-500">Dostępne zmienne</p>
