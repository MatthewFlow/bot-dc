    ...
    <div className="flex flex-col p-8 gap-8">
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="flex-1 rounded-xl bg-[#1a1f2e]">
          ...
          {[1,2,3].map((i) => (
            <div key={i} className="grid grid-cols-3 items-center border-b border-white/5 px-6 py-3">
...
export default function LevelsPage() {
  ...
  function addReward() {
    ...
    setNewLevel(""); setNewRoleId("");
  }

  function removeReward(level: number) {
    setConfig((c) => ({ ...c, roleRewards: (c.roleRewards ?? []).filter((r) => r.level !== level) }));
  }
  ...
  return (
    <div className="flex flex-col p-8 gap-8">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-gray-600">Growth Ladder</p>
        <h1 className="mt-1 text-2xl font-bold text-white">
          System <span className="italic text-[#d4a843]">levelowania</span>
        </h1>
        <p className="mt-1 text-sm text-gray-400">Mapowanie progów XP na istniejące role Discord.</p>
      </div>
        ...
        <div className="flex-1 rounded-xl bg-[#1a1f2e]">
          ...
          <div className="grid grid-cols-3 border-b border-white/5 px-6 py-2">
            {["Lv.", "Tier", "Discord rola"].map((h) => (
              <span key={h} className="text-xs font-semibold uppercase tracking-wider text-gray-600">{h}</span>
            ))}
          </div>

          {rewards.length === 0 ? (
            <div className="px-6 py-8 text-center text-sm text-gray-500">Brak progów. Dodaj pierwszy po prawej.</div>
          ) : (
            rewards.map((r) => (
              <div key={r.level} className="grid grid-cols-3 items-center border-b border-white/5 px-6 py-3 last:border-0">
                <span className="text-sm font-bold text-white">Lv. {r.level}</span>
                <span className="inline-flex">
                  <span className="rounded bg-[#d4a843]/20 px-2 py-0.5 text-xs font-semibold text-[#d4a843]">{roleName(r.roleId).toUpperCase()}</span>
                </span>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => removeReward(r.level)} className="text-gray-600 hover:text-red-400">✕</button>
                </div>
              </div>
            ))
          )}
          ...
          <div className="rounded-xl bg-[#1a1f2e] p-6">
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input type="number" min={1} value={newLevel} onChange={(e) => setNewLevel(e.target.value)} placeholder="np. 10"
              ...
              <button onClick={addReward} disabled={!newLevel || !newRoleId}
      ...
      <div className="rounded-xl bg-[#1a1f2e]">
        <div className="flex items-center justify-between border-b border-white/5 px-6 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Wszyscy aktywni członkowie</p>
        ...
        <div className="grid grid-cols-[2rem_1fr_6rem_6rem] gap-4 border-b border-white/5 px-6 py-2">
          {["#", "Gracz", "Level", "XP"].map((h, i) => (
            <span key={h} className={`text-xs font-semibold uppercase tracking-wider text-gray-600 ${i >= 2 ? "text-right" : ""}`}>{h}</span>
          ))}
        </div>

        {leaderboardLoading ? (
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="border-b border-white/5 last:border-0"><SkeletonRow /></div>
          ))
        ) : leaderboard.length === 0 ? (
          <div className="px-6 py-8 text-center text-sm text-gray-500">Brak danych XP na tym serwerze.</div>
        ) : (
          leaderboard.map((entry) => (
            <div key={entry.userId}
              ...
              <span className="text-sm font-bold text-gray-400">{MEDALS[entry.position - 1] ?? entry.position}</span>
              ...
              <div className="text-right">
                <span className="rounded bg-[#d4a843]/20 px-2 py-0.5 text-xs font-semibold text-[#d4a843]">Lv. {entry.level}</span>
              </div>
              <div className="text-right text-sm text-gray-300">{entry.xp.toLocaleString()}</div>
            </div>
          ))
        )}
      </div>
