...
- **Command dispatch**: `src/commands/handlers/handler.ts` routes interactions via a `Record<string, Handler>` map to `user.ts`, `admin.ts`, or `test.ts`. Admin commands are guarded by `guard.ts` (`requireAdminRole`) which checks `CFG_ADMIN_ROLE_ID` and replies ephemerally on failure.
...
- Routes:
  ...
  - `GET/PUT /guilds/:guildId/config` — guild config read/write; PUT uses an explicit allowlist (`CONFIG_ALLOWED_FIELDS` in `src/routes/guilds.ts`) — unknown fields are silently dropped
